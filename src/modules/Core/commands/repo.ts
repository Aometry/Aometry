    ...
    builder
      ...
      .addSubcommand((sub) =>
        sub
          ...
          .setDescription('Configure remote sync settings for an installed module')
          .addStringOption((opt) =>
            opt.setName('name').setDescription('Installed module name').setRequired(true)
          )
          .addStringOption((opt) =>
            opt.setName('remote').setDescription('Git HTTPS remote URL').setRequired(false)
          )
          .addStringOption((opt) =>
            opt.setName('branch').setDescription('Target branch name (default: main)').setRequired(false)
          )
          .addStringOption((opt) =>
            opt
              .setName('token-env')
      ...
      .execute(async ({ interaction, client }) => {
        ...
        if (subcommand === 'install') {
          ...
          if (!syncResult.success) {
            await interaction.editReply({ content: `❌ ${syncResult.message}` })
            return
          }
